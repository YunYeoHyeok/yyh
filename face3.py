...
# image 대상이미지(3채널)/ 대상이미지의 x,y 위치 / 가로 세로 크기 w, h / 덮어쓸 이미지(4채널)
# 4채널 이미지에서  alpha 값을 가져오기 위해
def overlay(image, x, y, w, h, overlay_image):
    alpha = overlay_image[:, :, 3] # BGRA
    mask_image = alpha/255 # 0~255 값 사이인데 255로 나누면 0~1사이의 mask이미지를 가진다 (1:불투명, 0: 투명)
    # ex) {255,255} = {1,1}
    # ex) {255,0} = {1,0}
    # 1-mask_image 를 하게된다면
    # {0.0}
    # {0.1} 서로 반전이 된다.

    
    for c in range(0, 3): # chanle / BGR
...
# 이미지 불러오기
image_right_eye = cv2.imread("C:\\yyh\\openCV\\data\\sunglass_right.png", cv2.IMREAD_UNCHANGED) # 이미지 크기 100x100
...
# FaceDetention 함수를 불러오고 with는 별도로 close 할 필요없이 자동으로 닫힌다
    ...
    while cap.isOpened():
        ...
        if results.detections:
            ...
            for detection in results.detections:
                ...
                print(detection) # detection 정보를 알려준다. score는 신뢰도
                ...
                # 양 눈에 동그라미 그리기
                #cv2.circle(image,right_eye,50,(255,0,0),10,cv2.LINE_AA) # 오른쪽 눈에 파란색
                #cv2.circle(image,left_eye,50,(0,255,0),10,cv2.LINE_AA) # 왼쪽 눈에 초록색

                # 코에 동그라미 그리기
                #cv2.circle(image,nose_tip,75,(0,255,255),10,cv2.LINE_AA) # 코는 노란색
